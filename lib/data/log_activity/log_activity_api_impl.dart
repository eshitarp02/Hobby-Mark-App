...
class LogActivityApiImpl extends LogActivityApi {
  ...
  Future<LogNewActivityResponseModel?> logNewActivity({
    ...
    try {
      final response = await apiClient.post(
        '${_appConfig.baseUrl}${ApiEndPoints.activityLog}',
  ...
  Future<ActivityLogResponseModel?> getActivityLog({
    ...
    try {
      final ActivityLogResponseModel activityLogResponseModel;
      final response = await apiClient.get(
        '${_appConfig.baseUrl}${ApiEndPoints.activityLog}?userId=$userId',
        options: Options(
          method: 'GET',
        ),
      );
      activityLogResponseModel = ActivityLogResponseModel.fromJson(
        response?.data,
      );
